...
function resize_and_crop($source_image_path, $thumbnail_image_path, $result_width, $result_height){
    ...
    if (!file_exists($source_image_path)) {
        $_SESSION['errors'][] = [   'title' => 'Ошибка масштабирования изображения.', 
                                    'desc' => 'Не был обнаружен путь к загруженному файлу. Необходимо проверить переменную, переданную в функцию resize_and_crop в файле addreview.php'];
        header("Location: " . HOST . "preferences");
        return false;
    }
    if (!getimagesize($source_image_path)) {
        $_SESSION['errors'][] = [   'title' => 'Ошибка масштабирования изображения.', 
                                    'desc' => 'Не были получены данные о расширении изображения.'];
        header("Location: " . HOST . "preferences");
        return false;
    }
